...
const AuthPopup = ({ closePopup }) => {
  ...
  const [isSignIn, setIsSignIn] = useState(true); // Détermine si l'utilisateur est en mode connexion ou inscription
  ...
  const [location, setLocation] = useState(null); // Stocke la localisation de l'utilisateur
  ...
  // Effet pour récupérer la localisation stockée et rediriger l'utilisateur
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      ...(formData.email && { email: formData.email }), // Inclure l'email uniquement si renseigné
      location,
    };

    try {
      const response = await axios.post(
        `${process.env.API_ENDPOINT}${endpoint}`, // Utilisation de la variable d'environnement
        payload
      );
